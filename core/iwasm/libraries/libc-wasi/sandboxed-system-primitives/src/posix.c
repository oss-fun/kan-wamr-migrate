...
static __wasi_errno_t
fd_prestats_get_entry(struct fd_prestats *pt,
                      __wasi_fd_t fd,
...
static __wasi_errno_t
fd_table_get_entry(struct fd_table *ft,
                   __wasi_fd_t fd,
                   __wasi_rights_t rights_base,
                   __wasi_rights_t rights_inheriting,
                   struct fd_entry **ret) REQUIRES_SHARED(ft->lock)
{
    // Test for file descriptor existence.
    if (fd >= ft->size)
        return __WASI_EBADF;
...
static void
fd_table_attach(struct fd_table *ft,
                __wasi_fd_t fd,
                struct fd_object *fo,
                __wasi_rights_t rights_base,
                __wasi_rights_t rights_inheriting) REQUIRES_EXCLUSIVE(ft->lock)
  CONSUMES(fo->refcount)
{
...
static __wasi_errno_t
fd_determine_type_rights(int fd,
                         __wasi_filetype_t *type,
                         __wasi_rights_t *rights_base,
                         __wasi_rights_t *rights_inheriting)
{
...
// descriptor table.
static __wasi_errno_t
fd_table_insert(struct fd_table *ft,
                struct fd_object *fo,
                __wasi_rights_t rights_base,
                __wasi_rights_t rights_inheriting,
...
static __wasi_errno_t
fd_table_insert_fd(struct fd_table *ft,
                   int in,
                   __wasi_filetype_t type,
                   __wasi_rights_t rights_base,
                   __wasi_rights_t rights_inheriting,
...
static __wasi_errno_t
fd_object_get_locked(struct fd_object **fo,
                     struct fd_table *ft,
                     __wasi_fd_t fd,
                     __wasi_rights_t rights_base,
                     __wasi_rights_t rights_inheriting)
...
static __wasi_errno_t
fd_object_get(struct fd_table *curfds,
              struct fd_object **fo,
              __wasi_fd_t fd,
              __wasi_rights_t rights_base,
              __wasi_rights_t rights_inheriting)
    ...
    *nread = (size_t)len;
    ...
    if (iovcnt == 1) {
        ...
        *nread = len;
        ...
        if (buf)
            alloc_info_buf(buf, uint8T, totalsize);
        if (buf == NULL) {
            fd_object_release(fo);
            return __WASI_ENOMEM;
        }
...
#ifndef BH_VPRINTF
    ssize_t len =
      writev(fd_number(fo), (const struct iovec *)iov, (int)iovcnt);
#else
    ssize_t len = 0;
...
readlinkat_dup(int fd, const char *path, size_t *p_len)
{
    ...
    for (;;) {
        ...
        if (newbuf)
            alloc_info_buf(newbuf, charT, len);
...
// directory.
static __wasi_errno_t
path_get(struct fd_table *curfds,
...
static __wasi_errno_t
path_get_nofollow(struct fd_table *curfds,
                  struct path_access *pa,
                  __wasi_fd_t fd,
                  const char *path,
                  size_t pathlen,
    ...
    for (i = 0; i < pt->size; i++) {
        if (pt->prestats[i].dir) {
            if (!(prestat_dir_real =
                ...
                return false;
            if (!strncmp(path_real, prestat_dir_real,
...
bool
argv_environ_init(struct argv_environ_values *argv_environ,
                  char *argv_buf,
                  size_t argv_buf_size,
                  char **argv_list,
                  size_t argc,
