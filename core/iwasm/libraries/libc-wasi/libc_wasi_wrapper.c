...
wasi_args_get(wasm_exec_env_t exec_env, uint32 *argv_offsets, char *argv_buf)
{
    ...
    if (total_size >= UINT32_MAX
        ...
        return (wasi_errno_t)-1;
    alloc_info_buf(argv, charTT, total_size);
                 ...
                 char *environ_buf)
{
    ...
    if (total_size >= UINT32_MAX
        ...
        return (wasi_errno_t)-1;
    alloc_info_buf(environs, charTT, total_size);
                    ...
                    wasi_prestat_app_t *prestat_app)
{
    ...
    wasi_ctx_t wasi_ctx = get_wasi_ctx(module_inst);
    struct fd_prestats *prestats =
      wasi_ctx_get_prestats(module_inst, wasi_ctx);
